...
//                $res['type'] = $this->type;
            ...
            if(!$pos){
                $posnew = strrpos($ipcountry,'内蒙古');
                if($posnew !== false){
                    $country = '中国';
                    $province = substr($ipcountry,0,$posnew+9);
                    $citypos=strrpos($ipcountry,'市');
                    if($citypos){
                        $city = substr($ipcountry,$posnew+9,$citypos+3-($posnew+9));
                    }else{
                        $city = '';
                    }
                }else{
                ...
                $country = '中国';
                $province = substr($ipcountry,0,$pos+3);
                $citypos=strrpos($ipcountry,'市');
                if($citypos){
                    $city = substr($ipcountry,$pos+3,$citypos-$pos);
                }else{
                    $city = '';
                }
            }
