...
class AdminNavModel extends BaseModel{
     ...
     * @return array        结构数据
     */
    public function getTreeData($type='tree', $order='', $user_id=''){
        // 判断是否需要排序
        if(empty($order)){
            $data=$this->select();
        }else{
        ...
        if($type=='tree'){
            ...
            if ($user_id !== 0) $user_id = $user_id ?: session('user.id');
            if ($user_id === 0 && C('PERMIT_ADMIN_LOGIN')===true) {
                // 超级管理员
                return $data;
            }
            if (!$user_id) return [];
            ...
            $auth=new \Think\Auth();
            foreach ($data as $k => $v) {
                if ($auth->check($v['mca'], $user_id)) {
                    foreach ($v['_data'] as $m => $n) {
                        if(!$auth->check($n['mca'], $user_id)){
